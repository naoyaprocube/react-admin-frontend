...
const FileInfo = () => {
  ...
  return filename ? (<Box sx={{ mb:2, bgcolor: blue[200], width: 1, p: 2, borderRadius: '16px', boxShadow: 1 }}>
    <Box>
      {translate('resources.files.fields.length')}: {length}
    </Box>
    <Box>
      {translate('resources.files.fields.metadata.status')}: {status}
    </Box>
  </Box>
  ) : null
};
  ...
  return (
    <Edit actions={false} title={<EditTitle />}>
      <SimpleForm >
        ...
        <FileInfo />
        <Box>
          {translate('resources.files.fields.metadata.accessHistory')}
        </Box>
